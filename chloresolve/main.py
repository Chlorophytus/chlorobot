        ...
        match command:
            case b"PRIVMSG":
                ...
                if chloresolve.strip.is_action(message_stripped):
                    message_stripped = chloresolve.strip.strip_action(message_stripped)
                    s_message: str = message_stripped.decode("utf-8", errors="ignore")
            ...
            case b"KICK":
                ...
                self.logger.info(f"[{channel}] {nickname} kicks {victim} ({reason})")
            case b"TOPIC":
                # gRPC endpoint does bytes
                [b_nickname, b_ident_cloak] = message.prefix.split(b"!", 1)
                [b_ident, b_cloak] = b_ident_cloak.split(b"@", 1)
                ...
                nickname: str = b_nickname.decode("utf-8")
                channel: str = b_channel.decode("utf-8")
                topic: str = message.trailing_parameter.decode("utf-8", errors="ignore")
