...
help = {
    ...
    markov = "generates nonsense with a Markov chain algorithm",
}
...
function commands.markov(hostinfo, destination, arguments)
    if permissions.get(hostinfo.cloak, 'markov') then
        if arguments[1] == "run" then
            if #arguments == 3 then
                local min = tonumber(arguments[2])
        ...
        elseif arguments[1] == "load" then
            if #arguments == 2 then
                markov_obj = markov_feed(arguments[2])
                chlorobot.respond(destination, hostinfo.nickname .. ": Markov object has been loaded")
            end
        end
    end
end
